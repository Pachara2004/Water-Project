...
export default function AdminLocationsPage() {
  ...
  const [organization, setOrganization] = useState('FISHERY');
  ...
  const handleEdit = async () => {
    if (!editingLoc || !editName.trim()) return;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh px-6 text-center w-full max-w-lg mx-auto bg-surface-muted border-x border-border">
        ...
        <p className="text-xs text-text-secondary mb-6 max-w-[80%] mx-auto leading-relaxed">
          หน้าการกำหนดค่าพิกัดพ้นกรอบการจัดการทั่วไป สำหรับผู้ดูแลระบบสูงสุด (System Admin) เท่านั้น
        </p>
        <button
  ...
  const uniqueOrgs = Array.from(new Set(locations.map(l => l.organization)));
    ...
    <div className="min-h-dvh w-full bg-surface-muted pb-[120px] relative transition-colors duration-300 overflow-hidden">
      ...
      <div className="px-4 sm:px-6 space-y-6 sm:space-y-10">
          ...
          <div className="space-y-7">
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full px-5 py-3.5 bg-surface-subtle border border-border text-text-primary rounded-2xl text-xs focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-none cursor-pointer min-h-[48px] appearance-none"
                >
                  <option value="FISHERY">กรมประมง</option>
                  <option value="POLLUTION">กรมควบคุมมลพิษ</option>
                  <option value="OTHER">หน่วยงานส่วนท้องถิ่น / อื่นๆ</option>
            ...
            <div className="space-y-4">
              ...
              <div className="w-full h-52 rounded-2xl overflow-hidden border border-border bg-surface-subtle relative">
                <MapView
                  mode="picker"
                  onLocationPick={(lat, lng) => setPickedPosition({ lat, lng })}
                  pickedPosition={pickedPosition}
                />
              </div>

              {pickedPosition ? (
                /* Coords badge in Blueprint layout style */
            ...
            <button
              ...
              disabled={!name.trim() || !pickedPosition || saving || (organization === 'CUSTOM' && !customOrg.trim())}
        ...
        <section className="space-y-7">
          ...
          {/* Location List - Solid flat panels for GPU speed */}
                  ...
                  {deletingId === loc.id ? (
                    <div className="p-6 bg-red-500/5 border-b border-red-500/10 space-y-5">
                      <p className="text-xs font-bold text-danger">
                        ⚠️ ลบสถานี &quot;{loc.name}&quot; พร้อมผลข้อมูลประวัติทิ้งถาวร?
                      </p>
      ...
      {editingLoc && (
        <>
          ...
          {/* Slide up Drawer */}
          <div className="fixed bottom-0 left-0 right-0 z-[801] bg-surface rounded-t-[32px] shadow-2xl border-t border-border max-w-lg mx-auto px-8 pt-8 space-y-8 animate-slide-up transition-colors duration-300" style={{ paddingBottom: 'calc(56px + env(safe-area-inset-bottom))' }}>
