...
export default function AdminUsersPage() {
  ...
  return (
    <div
      className="min-h-dvh w-full bg-surface-muted pb-[120px] transition-colors duration-300"
            ...
            >
              {t === "queue" ? (
                <>
                  ...
                  รอการอนุมัติ
                  {queue.length > 0 && (
                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-full ${tab === "queue" ? "bg-white/20 text-white" : "bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400"}`}>
                      {queue.length}
                    </span>
                  )}
                </>
          ...
          {loading ? (
            ...
            <div className="bg-surface rounded-3xl p-10 text-center border border-border">
              ...
              <p className="text-xs font-bold text-text-muted">
                {tab === "queue" ? "ไม่มีผู้ใช้รอการอนุมัติ" : "ไม่พบผู้ใช้ที่ค้นหา"}
              </p>
            </div>
          ) : (
            filtered.map((user) => {
                    ...
                    <div className="relative flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                      ...
                      {isOpen && (
                        <div className="absolute right-0 top-[calc(100%+6px)] z-50 bg-surface border border-border rounded-2xl shadow-xl py-2 min-w-[180px] animate-fade-in">
                          <p className="text-[9px] font-bold text-text-muted uppercase tracking-wider px-4 pt-1 pb-2 border-b border-border mb-1">
                            เลือกบทบาท
                          </p>
                          {ROLE_OPTIONS.map((r) => {
