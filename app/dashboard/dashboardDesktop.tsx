...
// ไม่เปลี่ยน logic/สี/การคำนวณ ใช้ hook และ helper ชุดเดียวกับ dashboardMobile
export default function DashboardDesktop() {
    ...
    return (
        <div className="min-h-dvh w-full bg-bg pb-8 antialiased transition-colors duration-300">
            <div className="w-full max-w-[1600px] mx-auto px-8 space-y-5 pt-8">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        ...
                        <div className="relative flex-1 max-w-md" ref={agencyMenuRef}>
                        ...
                        {userRole === "admin" && (
                            <div className="h-10 grid grid-cols-2 rounded-xl p-1 bg-card-general border border-border font-semibold text-center text-xs shrink-0">
                                <button
                                    ...
                                    className={`h-full px-4 rounded-lg transition-all cursor-pointer whitespace-nowrap ${viewMode === "ALL" ? "bg-secondary text-white shadow-xs" : "text-text-secondary bg-surface-subtle"}`}
                                >
                                    ข้อมูลทั้งหมด
                                </button>
                                <button
                                    onClick={() => setViewMode("MINE")}
                                    className={`h-full px-4 rounded-lg transition-all cursor-pointer whitespace-nowrap ${viewMode === "MINE" ? "bg-secondary text-white shadow-xs" : "text-text-secondary bg-surface-subtle"}`}
                                >
                                    ข้อมูลของฉัน
                                </button>
                            </div>
                        ...
                        <div className={`space-y-4 transition-opacity duration-200 ${!analytics ? "opacity-40 pointer-events-none" : "opacity-100"}`}>
                            ...
                            <div className="grid grid-cols-12 gap-2.5 shrink-0">
                                {analytics?.kpis?.map((kpi: any, index: number) => (
                                    <div
                                        key={index}
                                        className={`bg-card-summary rounded-xl border border-border p-3 flex flex-col border-l-10 ${kpiSpanClass(kpi.w)}`}
                            ...
                            <div className="grid grid-cols-12 gap-3">
                                ...
                                {!analytics?.stationDetail && (
                                    <div className="col-span-5 bg-surface rounded-xl border border-border p-4 flex flex-col overflow-hidden justify-between">
