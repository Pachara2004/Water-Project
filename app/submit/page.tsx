...
function SubmitContent() {
  ...
  const [exifData, setExifData] = useState<{ lat: number, lng: number } | null>(null);
      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          const sorted = [...allLocations].sort((a, b) => {
            const distA = calculateDistance(pos.coords.latitude, pos.coords.longitude, a.lat, a.lng);
            const distB = calculateDistance(pos.coords.latitude, pos.coords.longitude, b.lat, b.lng);
            return distA - distB;
          });
    ...
    if (file) {
      ...
      try {
        ...
        if (coords && allLocations.length > 0) {
          setExifData({ lat: coords.latitude, lng: coords.longitude });
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setResults({
        phosphate: typeof data.phosphate === 'number' ? data.phosphate : 0,
  ...
  const handleSave = async () => {
    if (!results || !currentLocationId || !currentUser) return;
    ...
    try {
      const res = await fetch('/api/samples', {
        ...
        body: JSON.stringify({
          ...
          phosphateVal: results.phosphate,
          ammoniaVal: results.ammonia,
      ...
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="bg-surface px-5 sm:px-8 lg:px-12 pt-12 pb-6 mb-6 sm:mb-8 border-b border-border transition-colors duration-300">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-text-primary tracking-wide leading-tight">
            ส่งผลตรวจ <span className="text-primary font-bold">ตัวอย่างน้ำชายฝั่ง</span>
          </h1>
          <p className="text-text-secondary text-xs sm:text-sm mt-3 leading-loose">
            อัปโหลดรูปภาพชุดทดสอบเคมีเพื่อทำการวิเคราะห์ข้อมูลด้วยระบบปัญญาประดิษฐ์ (AI)
          </p>
          ...
          {locationName ? (
            <div className="flex items-center justify-between gap-2.5 mt-4 text-text-secondary text-sm bg-primary-light border border-primary/10 py-2.5 px-4 rounded-2xl transition-colors duration-300">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="font-extrabold text-text-primary truncate">{locationName}</span>
              </div>
              <button
                onClick={() => {
            ...
            <div className="mt-4 space-y-3">
              ...
              {searchQuery.trim() ? (
                <div className="flex flex-col gap-1.5">
                  <p className="text-[9px] font-bold text-text-muted uppercase tracking-wide px-1 font-mono">Search Results</p>
                  {allLocations
                    ...
                    .map(loc => (
                      ...
                      >
                        <div className="w-8 h-8 bg-surface-subtle rounded-xl flex items-center justify-center group-hover:bg-primary-light transition-colors flex-shrink-0">
                          <MapPin size={13} className="text-text-muted group-hover:text-primary" />
                        </div>
                        <span className="text-xs font-bold text-text-primary flex-1 truncate">{loc.name}</span>
                      </button>
                    ))}
                  {allLocations.filter(loc => loc.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                    <p className="text-xs text-text-muted text-center py-3">ไม่พบจุดตรวจสอบที่ตรงกับคำค้น</p>
                  )}
                </div>
              ) : (
                ...
                <div>
                  {nearestLocations.length > 0 ? (
                    <>
                      <p className="text-[9px] font-bold text-text-muted uppercase tracking-wide px-1 mb-2.5 font-mono">📍 Nearby Recommendations</p>
                    ...
                    <div className="text-xs text-text-muted bg-surface-subtle p-3 rounded-2xl border border-border flex items-center gap-2">
                      ...
                      <span>กำลังค้นหาจุดตรวจแนะนำที่ใกล้ที่สุด...</span>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="bg-surface rounded-3xl shadow-md border border-border overflow-hidden transition-all duration-300">
              <div className="p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <h2 className="text-xs font-bold text-text-primary uppercase tracking-wider font-mono">Select Bottle Image</h2>
                </div>

                {/* Upload zone - High Contrast & clean corners */}
                <div
                  onClick={() => fileInputRef.current?.click()}
                  className={`relative w-full aspect-[4/3] rounded-3xl border-2 border-dashed cursor-pointer transition-all duration-300 overflow-hidden group ${imagePreview
                    ...
                    <div className="flex flex-col items-center justify-center h-full gap-3 px-6">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[10px] text-text-muted leading-relaxed mt-1 max-w-[80%] mx-auto">
                          โปรดให้แผ่นเปรียบเทียบค่าสี ColorChecker ในกล่องทดสอบมีความชัดเจน
                        </p>
                      </div>
                ...
                <div className="mt-5 space-y-2">
                  <label className="text-[9px] font-bold text-text-muted uppercase tracking-wider block">
                    เวลาที่เก็บตัวอย่างน้ำจริง (Collection Time)
                  </label>
                  <input
                    type="datetime-local"
                  ...
                  <p className="text-[8px] text-text-muted leading-relaxed">
                    *ระบุเวลาจริงที่ลงพื้นที่เก็บตัวอย่าง เพื่อดึงข้อมูลสภาพอากาศ TMD ย้อนหลังได้ถูกต้อง
                  </p>
                </div>
                ...
                >
                  {isRecommending ? (
                    ...
                    <>
                      <Sparkles size={15} className={imageFile && currentLocationId ? "animate-pulse" : ""} />
                      {!currentLocationId && imageFile ? "กรุณาเลือกจุดตรวจสอบ" : "เริ่มการวิเคราะห์ด้วย AI"}
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              {imagePreview && (
                <div className="bg-surface rounded-3xl shadow-md border border-border overflow-hidden p-2 transition-all duration-300">
                  <img src={imagePreview} alt="ตัวอย่างน้ำที่วิเคราะห์" className="w-full h-44 object-cover rounded-2xl" />
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-surface rounded-2xl p-5 shadow-md border border-border flex flex-col justify-between hover:scale-[1.01] active:scale-[0.99] transition-all duration-200">
                  ...
                  <div className="flex items-baseline gap-1">
                    <div className={`text-2xl font-black ${(() => {
                        ...
                        return getParameterStatus(results.phosphate, std.phosphateMax) === 'SAFE' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400';
                      })()
                      }`}>
                      {(results.phosphate ?? 0).toFixed(3)}
                    </div>
                ...
                <div className="bg-surface rounded-2xl p-5 shadow-md border border-border flex flex-col justify-between hover:scale-[1.01] active:scale-[0.99] transition-all duration-200">
                  ...
                  <div className="flex items-baseline gap-1">
                    <div className={`text-2xl font-black ${(() => {
                        const std = LOCATION_STANDARDS[locationType as keyof typeof LOCATION_STANDARDS] || LOCATION_STANDARDS['COMMUNITY'];
                        return getParameterStatus(results.ammonia, std.ammoniaMax) === 'SAFE' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400';
                      })()
                      }`}>
              ...
              <div className="bg-surface rounded-3xl shadow-md border border-border p-5 transition-all duration-300">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-[8px] text-text-muted leading-relaxed">
                    *ข้อมูลออกซิเจนละลายน้ำนี้เป็นแบบระบุเลือกได้ (Non-blocking) เพื่อใช้เป็นตัวแปรพยากรณ์โครงสร้างคณิตศาสตร์ระบบในภายหลัง
                  </p>
                </div>
              </div>
              ...
              <div className="bg-surface rounded-2xl shadow-md border border-border p-5 transition-all duration-200">
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {getStandardsEvaluation(results.phosphate, results.ammonia).map((std) => (
                    <div
                      key={std.type}
                      className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold border transition-all ${std.passed
                    ...
                    >
                      ...
                      <span className={`font-mono text-[9px] font-black px-2 py-0.5 rounded-lg border flex-shrink-0 ${std.passed
              ...
              {!saved ? (
                ...
                <div className="bg-emerald-50/40 dark:bg-emerald-950/20 border border-emerald-500/20 rounded-3xl p-5 text-center transition-all duration-300">
                  ...
                  <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300">บันทึกข้อมูลสำเร็จ!</p>
