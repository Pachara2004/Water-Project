                ...
                {item.statusRequest === "pending" && (
                    <div className="flex items-stretch gap-2 w-full">
                        ...
                        <button
                            ...
                            className="flex-1 min-h-9 px-2 rounded-xl bg-secondary hover:bg-secondary/90 text-white text-xs font-semibold sm:font-medium flex items-center justify-center gap-1.5 transition-all cursor-pointer disabled:opacity-50"
                        >
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-1">
                                        {s.measurements.map((m) => (
                                            <span
                                                key={m.parameterId}
                                                className={`text-xs px-1.5 py-0.5 rounded font-medium ${isLowConfidence(m.confidence) ? "text-text-danger" : "text-text-safe"}`}
                                                title={`ค่าความมั่นใจของ AI (เกณฑ์ขั้นต่ำ ${CONFIDENCE_THRESHOLD.toFixed(2)})`}
                                            >
                            ...
                            {item.samples
                                ...
                                .map((m) => (
                                    <div key={m.parameterId} className="flex-1 min-w-50 flex items-center justify-between gap-2 bg-card-general border border-primary/30 rounded-sm px-3 py-2">
                                        ...
                                        <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${isLowConfidence(m.confidence) ? "text-text-danger" : "text-text-safe"}`}>
                                            ค่าความมั่นใจ {m.confidence.toFixed(2)}
                                        </span>
                                    </div>
                                ))}
                ...
                {item.statusRequest === "pending" && (
                    <div className="flex items-stretch gap-2 w-full mt-1">
                        ...
                        <button
                            ...
                            disabled={actingId === item.id || noneSelected}
                            onClick={() => onApprove(item, showSampleSelect ? selectedSampleIds : undefined)}
                            className="flex-1 min-h-10 px-2 rounded-lg bg-secondary hover:bg-primary text-white text-xs font-semibold sm:font-medium flex items-center justify-center gap-1.5 transition-all cursor-pointer disabled:opacity-50"
                        >
