        ...
        <div ref={containerRef} className="relative w-full z-700">
            ...
            <div className="flex items-center w-full h-12 px-4 bg-card-general rounded-xl shadow-sm transition-all">
                ...
                <input
                    ...
                    placeholder="ค้นหาชื่อสถานที่หรือหน่วยงาน..."
                    className="w-full h-7 bg-transparent text-xs font-semibold text-black  placeholder:text-text-muted/60"
                />
                {keyword && (
                    <button
                        type="button"
            ...
            {isOpen && filtered.length > 0 && (
                <div className="absolute top-[calc(100%+6px)] left-0 w-full bg-surface border border-border rounded-xl shadow-lg max-h-56 overflow-y-auto py-1">
                    {filtered.map((loc) => (
                        <button
                            key={loc.id}
                            type="button"
                        ...
                        >
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="text-xs text-black truncate mt-0.5">{loc.organization}</div>
                            </div>
                        </button>
                    ))}
                </div>
