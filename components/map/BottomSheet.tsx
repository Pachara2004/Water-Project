...
export default function BottomSheet({ location, onClose }: BottomSheetProps) {
    ...
    const getSnapHeight = (snap: "collapsed" | "half" | "full"): number =>
        HEIGHTS[snap];
    ...
    const getNearestSnapPoint = (
        ...
        return (
            Object.keys(dists) as Array<"collapsed" | "half" | "full">
    ...
    const handleDragMove = (clientY: number) => {
        ...
        const newHeight = Math.max(
            HEIGHTS.collapsed,
            Math.min(HEIGHTS.full, dragBaseHeightRef.current + delta),
        );
    ...
    const renderCollectorInfo = () => {
        ...
        if (role === "COLLECTOR") {
            ...
            if (isSelf) {
                return (
                    <div className="bg-surface border border-border rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-1 mt-4">
                        <span className="text-[9px] font-black text-text-muted uppercase tracking-wider block">
                            ผู้บันทึกข้อมูล (ข้อมูลของคุณเอง)
                        </span>
                        <p className="text-xs font-bold text-text-primary mt-1">
                            {colName}
                        </p>
                ...
                const anonymizedName =
                    colName.length > 0 ? `${colName[0]}***` : "***";
                return (
                    <div className="bg-surface border border-border rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-1 mt-4">
                        <span className="text-[9px] font-black text-text-muted uppercase tracking-wider block">
                            ผู้บันทึกข้อมูล (ผู้ใช้รายอื่น)
                        </span>
                        <p className="text-xs font-bold text-text-primary mt-1">
                            {anonymizedName}
                        </p>
                        <p className="text-[10px] text-text-muted italic">
                            เบอร์โทรศัพท์ถูกปกปิดตามความคุ้มครองข้อมูลส่วนบุคคล
                            (PDPA)
                        </p>
                    </div>
                );
        ...
        if (role === "EXECUTIVE") {
            ...
            return (
                <div className="bg-surface border border-border rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-1 mt-4">
                    <span className="text-[9px] font-black text-text-muted uppercase tracking-wider block">
                        ผู้บันทึกข้อมูล (ข้อมูลอนามัย)
                    </span>
                    <p className="text-xs font-bold text-text-primary mt-1">
                        {anonymizedName}
                    </p>
                    <p className="text-[10px] text-text-muted italic">
                        ข้อมูลส่วนบุคคลได้รับการปกปิดเพื่อความปลอดภัยของข้อมูล
                    </p>
                </div>
            );
        }
    ...
    const prev =
        samplesArr.length > 1 ? samplesArr[samplesArr.length - 2] : null;

    const po4Diff =
        prev && latest?.phosphateVal !== null && prev.phosphateVal !== null
            ? (latest?.phosphateVal ?? 0) - (prev.phosphateVal ?? 0)
            : 0;
    const nh3Diff =
        prev && latest?.ammoniaVal !== null && prev.ammoniaVal !== null
            ? (latest?.ammoniaVal ?? 0) - (prev.ammoniaVal ?? 0)
            : 0;
    ...
    const standardsEval = latest
        ? evaluateAllStandards(latest.phosphateVal, latest.ammoniaVal)
        : null;
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-text-secondary text-xs">
                        ...
                        <span className="font-bold">
                            {getOrganizationLabel(location.organization)}
                        </span>
                    </div>
                </div>
            </div>
            ...
            {latest ? (
                <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-surface rounded-2xl p-6 border border-border shadow-sm flex flex-col justify-between hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
                            ...
                            {prev && po4Diff !== 0 && (
                                <div
                                    className={`flex items-center gap-1 mt-2 text-[10px] font-black ${po4Diff > 0 ? "text-red-500" : "text-emerald-500"}`}
                                >
                                    {po4Diff > 0 ? (
                                        <TrendingUp size={10} />
                                    ) : (
                        ...
                        <div className="bg-surface rounded-2xl p-6 border border-border shadow-sm flex flex-col justify-between hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
                            ...
                            {prev && nh3Diff !== 0 && (
                                <div
                                    className={`flex items-center gap-1 mt-2 text-[10px] font-black ${nh3Diff > 0 ? "text-red-500" : "text-emerald-500"}`}
                                >
                                    {nh3Diff > 0 ? (
                                        <TrendingUp size={10} />
                                    ) : (
                    ...
                    {latest.oxygen !== null && latest.oxygen !== undefined && (
                        <div className="bg-surface rounded-2xl p-6 border border-border shadow-sm flex flex-col justify-between hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-[9px] font-black text-text-muted uppercase tracking-wider block">
                                    Dissolved Oxygen (DO) - ค่าออกซิเจนละลายน้ำ
                                </span>
                            </div>
                            <div className="flex items-baseline gap-1.5 mt-2">
                                <span className="text-2xl font-black text-text-primary">
                                    {latest.oxygen.toFixed(2)}
                                </span>
                                <span className="text-[10px] text-text-muted font-bold">
                                    mL/L
                                </span>
                    ...
                    {(latest.temperature !== null ||
                        ...
                        <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="grid grid-cols-3 gap-3 text-center">
                                {latest.temperature !== null &&
                                    latest.temperature !== undefined && (
                                        <div className="bg-surface-subtle p-3 rounded-xl border border-border">
                                            <span className="text-[8px] font-bold text-text-muted block uppercase">
                                                อุณหภูมิ
                                            </span>
                                ...
                                {latest.rainVolume !== null &&
                                    latest.rainVolume !== undefined && (
                                        <div className="bg-surface-subtle p-3 rounded-xl border border-border">
                                            <span className="text-[8px] font-bold text-text-muted block uppercase">
                                                ปริมาณฝน
                                            </span>
                                            <span className="text-xs font-black text-text-primary mt-1 block">
                                                {latest.rainVolume.toFixed(1)}{" "}
                                                mm
                                            </span>
                                        </div>
                                    )}
                                {latest.weatherCondition !== null &&
                                    latest.weatherCondition !== undefined && (
                                        <div className="bg-surface-subtle p-3 rounded-xl border border-border">
                                            ...
                                            <span
                                                className="text-[10px] font-black text-text-primary mt-1 block truncate"
                                                title={getWeatherConditionLabel(
                                                    latest.weatherCondition,
                                                )}
                                            >
                                                {latest.weatherCondition === 1
                                                    ? "Clear"
                                                    : latest.weatherCondition ===
                                                        8
                                                      ? "Thunderstorm"
                    ...
                    {modeStatus && samplesArr.length > 1 && (
                        <div className="flex items-center justify-center gap-2 py-1 text-xs text-text-secondary">
                            ...
                            <StatusBadge
                                status={
                                    modeStatus as "SAFE" | "WARNING" | "DANGER"
                                }
                                size="sm"
                            />
                            <Minus size={12} className="text-border" />
                    ...
                    {standardsEval && (
                        <div className="bg-surface-subtle border border-border rounded-2xl p-6">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {Object.entries(standardsEval).map(
                                    ([type, passed]) => (
                                        <div
                                            key={type}
                    ...
                    {chartData.length > 0 && (
                        <div className="bg-surface rounded-2xl p-6 border border-border shadow-sm">
                            <h4 className="text-[9px] font-black text-text-muted uppercase tracking-wider mb-4 px-1">
                                แนวโน้มค่าสารเคมี (WEEKLY TREND)
                            </h4>
                            ...
                            <div className="space-y-3">
                                {[...samplesArr]
                                    ...
                                    .map((s, idx) => (
                                        <div
                                            key={idx}
                                            className="flex justify-between items-center text-xs bg-surface border border-border px-4 py-3 rounded-xl"
                                        >
                                            ...
                                            <div className="flex items-center gap-4">
                                                <span className="text-[9px] text-text-muted font-mono bg-surface-subtle px-1.5 py-0.5 border border-border rounded">
                                                    P:{" "}
                                                    {s.phosphateVal?.toFixed(
                                                        2,
                                                    ) || "-"}{" "}
                                                    | A:{" "}
                                                    {s.ammoniaVal?.toFixed(2) ||
                                                        "-"}
                                                </span>
                                                <StatusBadge
                                                    status={s.status}
                ...
                <div className="text-center py-8">
                    ...
                    <p className="text-xs font-bold text-text-muted">
                        ยังไม่พบประวัติผลการวิเคราะห์ในพิกัดนี้
                    </p>
                </div>
            )}
            ...
            {currentUser &&
                (currentUser.role === "ADMIN" ||
                    ...
                    <div className="mt-8">
                        <button
                            onClick={() =>
                                router.push(`/submit?locationId=${location.id}`)
                            }
