...
export default function FilterBar({ value, onChange }: FilterBarProps) {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute top-[calc(100%+6px)] left-0 w-full*2 bg-card-general backdrop-blur-md rounded-2xl shadow-2xl border border-border/80 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 z-700 will-change-transform">
                    ...
                    <div className="p-2.5 border-b border-border bg-bg flex items-center gap-2 sticky top-0 z-10">
                        ...
                        {searchQuery && (
                            <button
                                ...
                                className="text-text hover:text-text p-1 rounded-lg hover:bg-bg transition-colors"
                            >
                                <X size={13} />
                            </button>
                        )}
                    ...
                    <div className="p-1 flex flex-col gap-0.5 h-full*2 overflow-y-auto overscroll-contain reserve-scrollbar-gutter scrollbar-thin" onTouchMove={(e) => e.stopPropagation()}>
                        {filteredOptions.length === 0 ? (
                            <div className="text-center py-6 text-xs text-text font-medium">ไม่พบข้อมูลหน่วยงานนี้</div>
                        ) : (
